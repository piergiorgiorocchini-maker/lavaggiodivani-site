/*
  LavaggioDivani cookie consent banner
...
(function () {
  ...
  var STORAGE_KEY = "lavaggiodivani_cookie_consent_v1";
  ...
  function applyConsent(consent) {
    ...
    if (
      window.LavaggioDivaniTracking &&
      typeof window.LavaggioDivaniTracking.updateConsent === "function"
    ) {
      window.LavaggioDivaniTracking.updateConsent(buildGoogleConsent(consent));
    }

    window.dispatchEvent(new CustomEvent("lavaggiodivani:cookie-consent-updated", {
      detail: {
        analytics: !!consent.analytics,
        marketing: !!consent.marketing
      }
  ...
  function injectStyles() {
    if (document.getElementById("ld-cookie-consent-styles")) return;

    var style = document.createElement("style");
    style.id = "ld-cookie-consent-styles";
    style.textContent = [

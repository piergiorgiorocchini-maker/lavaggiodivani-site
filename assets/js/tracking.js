/*
  LavaggioDivani central tracking loader
...
(function () {
  ...
  function log() {
    ...
    try {
      console.log.apply(console, ["[LavaggioDivani tracking]"].concat(Array.prototype.slice.call(arguments)));
    } catch (error) {}
  }
  ...
  function warn() {
    if (!state.config || !state.config.debug) return;
    try {
      console.warn.apply(console, ["[LavaggioDivani tracking]"].concat(Array.prototype.slice.call(arguments)));
    } catch (error) {}
  }
  ...
  function exposeConsentHelpers() {
    window.LavaggioDivaniTracking = window.LavaggioDivaniTracking || {};

    window.LavaggioDivaniTracking.updateConsent = function (consentUpdate) {
      if (typeof window.gtag !== "function") {
    ...
    window.LavaggioDivaniTracking.grantAnalyticsOnly = function () {
      window.LavaggioDivaniTracking.updateConsent({
        analytics_storage: "granted"
      });
    };

    window.LavaggioDivaniTracking.grantAllConsent = function () {
      window.LavaggioDivaniTracking.updateConsent({
        ad_storage: "granted",
    ...
    window.LavaggioDivaniTracking.denyAllConsent = function () {
      window.LavaggioDivaniTracking.updateConsent({
        ad_storage: "denied",
        ad_user_data: "denied",
    ...
    window.LavaggioDivaniTracking.pushEvent = pushEvent;
  }
  ...
  function loadConfig() {
    fetch(CONFIG_PATH, { cache: "no-store" })
      ...
      .catch(function (error) {
        ...
        try {
          console.warn("[LavaggioDivani tracking] Tracking config not loaded:", error.message);
        } catch (ignore) {}
      });
  }
